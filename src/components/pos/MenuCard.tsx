...
function MenuCardBase({ item, onAdd }: { item: MenuItem; onAdd: () => void }) {
  ...
  return (
    <div
      ...
      className="group relative overflow-hidden rounded-2xl glass-strong hover:border-primary/30 sm:hover:-translate-y-1 transition-transform duration-200 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/70"
    >
      <div className="relative h-36 overflow-hidden sm:aspect-square sm:h-auto">
        <img
          src={item.image}
          alt={item.name}
          loading="lazy"
      ...
      <div className="p-3 sm:p-4 sm:pt-3">
        <div className="flex items-start justify-between gap-2">
          <h3 className="font-semibold text-sm leading-tight min-w-0">{item.name}</h3>
