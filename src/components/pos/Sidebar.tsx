...
const NAV = [
  ...
  { icon: Globe, label: "Online Orders", to: "/online-orders" as const },
...
export function Sidebar() {
  ...
  const UserBlock = ({ compact = false }: { compact?: boolean }) =>
    me && (
      <div className={`glass rounded-xl p-3 mt-2 ${compact ? "hidden xl:block" : ""}`}>
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          >
            <button
              onClick={() => setMobileOpen(false)}
              className="absolute top-3 right-3 size-9 grid place-items-center rounded-lg hover:bg-white/5 text-muted-foreground"
